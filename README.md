...
## 사용방법
1. 검색창에 확인하고싶은 임상실험 병명을 입렵한다.
2. 검색하는도중 자동완성기능으로 추천 검색어가 나오게되면 마으수로 선택하거나 키보드로 선택할 수있다.
3. 자동완성을 선택하지 않고 검색을눌러도 해당 검색결과를 확인할 수 있다.
4. 자동완성 결과를 선택하면 선택과동시에 해당 검색결과를 새창으로 확인 할 수 있다.
...
> 윈도우 이벤트 리스너를 사용하여 키보드 화살표 이벤트 발생시 전역상태인 `selectList` 상태가 변경되도록 구현.<br/>
> 자동 완성리스트에 마우스를 올리거나 위아래 화살표를 했을때 선택된 항목을 알아볼수 있도록 포커스 기능구현.
> 화살표를 이용하여 이동시 리스트 첫번째나 마지막 이상으로 인덱스 값이 변하지 않도록 구현.
```jsx
//main.js 일부
  useEffect(() => {
